...
public class TaskService {
    ...
    public static void getIncompleteTasks() {
        ...
        ArrayList<Entity> allTasks = Database.getAll(Task.TASK_ENTITY_CODE);
        ArrayList<Task> incompleteTasksSort = new ArrayList<>();

        for(Entity entity : allTasks)
        {
            if( ((Task) entity).status != Task.Status.Completed)
                incompleteTasksSort.add( (Task) entity);
        }

        Collections.sort(incompleteTasksSort,
                new Comparator<Task>() {
                    @Override
                    public int compare(Task o1, Task o2) {
                        return o1.dueDate.compareTo(o2.dueDate);
        ...
        for(Task task : incompleteTasksSort)
        {
            DateFormat dateformat = new SimpleDateFormat("yyyy-MM-dd");
